...
- [ ] T018 [P] [US1] Create `src/components/Thumbnail.tsx` rendering the SVG produced by `src/lib/thumbnail.ts`
- [ ] T019 [US1] Create `src/components/GameCard.tsx` showing thumbnail, title, blurb, and price, with a buy control that is disabled with a visible reason when unaffordable and replaced by an `Owned` indicator when owned (FR-005, FR-009, FR-010)
- [ ] T020 [US1] Create `src/components/StoreView.tsx` rendering the listing grid for the active storefront, including an all-owned empty state
...
- [ ] T022 [US1] Add immediate visual purchase feedback in `src/components/GameCard.tsx` (FR-013)
...
- [ ] T027 [US2] Create `src/components/WorkView.tsx`: puzzle gate, countdown, live-falling balance, and the shift's hour cost shown both at rest and spaced out **before** committing (FR-038, FR-040, FR-049)
- [ ] T028 [US2] Create `src/components/SpaceOutButton.tsx`: a hold-to-activate control dispatching `SET_SPACING_OUT`, that cannot latch or be automated, and that releases on pointer-up, window blur, and view change (FR-051, FR-053, FR-055)
...
- [ ] T034 [P] [US3] Create `src/components/Announcement.tsx` as a shared toast for sale and release events, auto-dismissing after a few seconds and manually dismissible (FR-023, FR-024)
- [ ] T035 [US3] Add discount badge, struck-through original price, and sale price to `src/components/GameCard.tsx` (FR-026)
...
- [ ] T037 [US4] Add a storefront selector to `src/components/StoreView.tsx` dispatching `SET_STOREFRONT` (FR-007)
- [ ] T038 [US4] Apply per-storefront theming via CSS custom properties from `src/data/catalogue.ts` so each store is distinguishable at a glance (FR-006, SC-008)
...
- [ ] T040 [P] [US5] Create `src/components/LibraryView.tsx` listing owned games with title and thumbnail, plus an empty state (FR-029)
- [ ] T041 [P] [US5] Create `src/components/HistoryView.tsx` listing transactions chronologically with game, storefront, price paid, discount, and timestamp, plus an empty state (FR-030)
- [ ] T042 [US5] Show the running total of hours spent on games in `src/components/HistoryView.tsx` (FR-031)
...
- [ ] T044 [US6] Create `src/components/EndScreen.tsx` handling the death variant with games bought, hours spent, and shifts worked, blocking all purchases and shifts (FR-032)
